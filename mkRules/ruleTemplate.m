function varargout = ruleTemplate(method,varargin)
	...
	case 'enzQSSA'
		if expComp
			...
			tensVal   = {[subIndx     compIndx      -k*overlap*(1./x.comp(subIndx));
						 ...
						 [subIndx   subIndx enzIndx  1/Km;
						  subIndx   enzIndx subIndx  1/Km;
						  enzIndx   subIndx enzIndx  1/Km;
						  enzIndx   enzIndx subIndx  1/Km;
						  compIndx  subIndx enzIndx -1/Km;
						  compIndx  enzIndx subIndx -1/Km]};
			catch msg
				printErr(msg)
				keyboard
			end
...
for ii = 1:length(model.param)
	freeInd = ~isnan(model.param(ii).pInd);
