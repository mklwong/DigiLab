...
while existFile
	rePlc = input('Proposal Distribution file already exists. Replace (y/n)? ','s');
	if strcmpi(rePlc,'y')
...
elseif ~isempty(strfind(curDir,'/'))
	...
	error('mkProp: How did I get here? Please contact developer!!!!!')
end
curDir(rmIndx+1:end) = [];

if nargin == 2
    curDir = [curDir 'propTemplate_clean.m'];
else
    curDir = [curDir 'propTemplate.m'];
end
